...
class TagRepository
{
    ...
    public function getTagsByPageId(int $pageId, string $locale): Selection
    {
      return $this->findAll()
      ->select(Utils::createPath(
        ['*', self::TABLE_PRIMARY],
        [self::COLUMN_TRANSLATION_TITLE, self::TABLE_TRANSLATION, true],
        [self::COLUMN_TRANSLATION_SLUG, self::TABLE_TRANSLATION, true],
      ))
      ->where(self::COLUMN_TRANSLATION_LOCALE, $locale)
      ->where(Utils::createPath([PageRepository::COLUMN_TAG_PARENT, PageRepository::TABLE_TAG, true]), $pageId)
      ->order(Utils::createPath([self::COLUMN_TRANSLATION_TITLE, self::TABLE_TRANSLATION, true]), 'ASC');
    }

    public function findPageHasTag(): Selection
    {
      return $this->database->table(PageRepository::TABLE_TAG);
    }

    public function findProjectHasTag(): Selection
    {
      return $this->database->table(ProjectRepository::TABLE_TAG);
    }
